...
class LabelsManager:
    def __init__(self, master, canvas, secframe, accounts, scrollbar, notifications, on_mousewheel, accounts_frame, add_account_tab):
                ...
                if not self.edit_window_open:
                    ...
                    def edit_name():
                        ...
                        if not self.edit_name_window_open:
                            ...
                            #Verifies that name is <30 characters
                            ...
                            def save_name():
                                ...
                                if validate_name(new_name) and new_name != name:
                                    if new_name in self.accounts:
                                        override = messagebox.askyesno("Override Account", f"An account with the name '{new_name}' already exists. Do you want to override it?")
                                        if override:
                                            del self.accounts[new_name]
                                        else:
                                            count = 1
                                            while f"{new_name} ({count})" in self.accounts:
                                                count += 1
                    ...
                    delete_account_button = ttk.Button(edit_frame, text="Delete Account", command=lambda name=name: delete_account(name),
                                                        style='Red.TButton')
