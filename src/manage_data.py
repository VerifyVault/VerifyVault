...
class ManageData:
    ...
    def import_data(self):
        ...
        def import_from_qr():
            ...
            self.close_active_window()
            try:
                # Load existing accounts from data.vv if it exists
                existing_accounts = {}
                if os.path.exists("data.vv"):
                    key = backend.load_key()
                    existing_accounts = backend.load_accounts(key)

                # Ask user to select QR code image file
                    ...
                    with open(file_path, 'rb') as img_file:
                        ...
                        if result:
                            for data in result:
                                ...
                                account_name = parts[0].split('/')[-1].rstrip('?')  # Remove '?' from end of account name
